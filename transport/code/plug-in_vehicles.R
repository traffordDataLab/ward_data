...
df <- read_csv("https://assets.publishing.service.gov.uk/government/uploads/system/uploads/attachment_data/file/1090509/df_VEH0145.csv") %>%
  clean_names()
# LSOA to ward lookup #
...
df_ward <- left_join(df %>% clean_names(), lookup, by = "lsoa11cd") %>%
  ...
  mutate(indicator = "All licensed plug-in vehicles", period="2022-03",unit ="vehicles", measure = "Count") %>%
  select(area_code = wd18cd, area_name = wd18nm, indicator, period, measure, unit, value)

write_csv (df_ward, "../data/plug-in_vehicles.csv")
