# Deprivation: Household deprivation - Health and disability, 2021 #

# Source: Census 2021
# URL: https://www.ons.gov.uk/census/census2021dictionary/variablesbytopic/demographyvariablescensus2021/householddeprivedinthehealthanddisabilitydimension
...
for(i in 1:21) {
  ...
  url <- paste0("https://api.beta.ons.gov.uk/v1/population-types/HH/census-observations?area-type=oa,",  URLencode(paste0(paste(ward_oas %>% pull(OA21CD), collapse = ","))), "&dimensions=hh_deprivation_health")
...
df_dep_h_d <- df %>%
  ...
  filter(category == "Household is deprived in the health and disability dimension") %>%
  mutate(period = "2021",
         indicator = "Household is deprived in health and disability (from best-fit OAs)",
         measure = "Percentage",
         unit = "Households") %>%
...
write_csv(df_dep_h_d, "../data/household_health_deprived.csv")  
