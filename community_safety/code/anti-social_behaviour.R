# Community Safety: Anti-social behaviour, 2017-11 to 2018-10 #
...
pop <- read_excel("SAPE20DT8-mid-2017-ward-2017-syoa-estimates-unformatted.xls", sheet = 4, skip = 3) %>%
  filter(`Local Authority` == 'Trafford') %>%
  select(area_code = `Ward Code 1`, pop = `All Ages`)
...
df <- dir(pattern = "*.csv") %>%
  ...
  mutate(value = round((n/pop)*1000, 1),
         period = "2017-10 to 2018-11",
         indicator = "Anti-social behaviour",
         measure = "rate",
         unit = "incidents") %>%
  select(area_code, area_name, indicator, period, measure, unit, value)

write_csv(df, "../data/anti-social_behaviour.csv")
