# Demographics: Percentage of population aged 20-44 years, 2020 #
...
row <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_2010_1.data.csv?geography=1656750701...1656750715,1656750717,1656750716,1656750718...1656750721&date=latest&gender=0&c_age=200,6...10&measures=20100") 
...
df <- row %>%
  ...
  mutate(Percentage = round(rowSums(select(., `Aged 20-24`:`Aged 40-44`))/`All Ages` *100, 1),
      Count = rowSums(select(., `Aged 20-24`:`Aged 40-44`))) %>%
  select(area_code, area_name, Percentage, Count) %>%
