# Demographics: Percentage of population aged 20-44 years, 2021 #
...
df <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_2020_1.data.csv?date=latest&geography=629174437...629175131,629304895...629304912,629315184...629315186,629315192...629315198,629315220,629315233,629315244,629315249,629315255,629315263,629315265,629315274,629315275,629315278,629315281,629315290,629315295,629315317,629315327&c2021_age_19=0,5...9&measures=20100") %>%
  ...
  mutate(category= ifelse(category != "Total", "Aged 20-44 years", category)) %>%
  group_by(period,area_code, area_name, category) %>%
  summarise(value = sum(value)) %>%
  mutate(Percentage = round(value *100/value[2],1)) %>%
  ungroup() %>%
  filter(category == "Aged 20-44 years") %>%
  mutate(indicator = "Population aged 20-44 years (from best-fit OAs)",
         measure = "Percentage",
         unit = "Persons") %>%
