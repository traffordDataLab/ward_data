# Demographics: Percentage of population aged 65 or more years, 2020 #
...
df <- row %>%
  ...
  mutate(Percentage = round(`Aged 65+`/`All Ages`*100, 1)) %>% 
  select(area_code, area_name, Percentage, Count = `Aged 65+`) %>%
