...
# Source: TS023 - Multiple ethnic group
# URL: https://nmweb.dur.ac.uk/datasets/c2021ts023
...
df <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_2042_1.data.csv?date=latest&geography=629174437...629175131,629304895...629304912,629315184...629315186,629315192...629315198,629315220,629315233,629315244,629315249,629315255,629315263,629315265,629315274,629315275,629315278,629315281,629315290,629315295,629315317,629315327&c2021_meth_6=0,2&measures=20100") %>%
  select(period = DATE_NAME,
         OA21CD = GEOGRAPHY_CODE,
         category = C2021_METH_6_NAME,
         value = OBS_VALUE) %>%
  ...
  ungroup() %>%
  filter(category == "All household members have the same ethnic group") %>%
  mutate(indicator = "Single ethnicity households with two or more members (from best-fit OAs)",
         measure = "Percentage",
         unit = "Households") %>%
