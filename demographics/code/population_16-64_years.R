# Demographics: Percentage of population aged 16-64 years, 2021 #
...
df_16_64 <- df %>%
  ...
  filter(category == "Aged 16 to 64 years") %>%
  mutate(period = "2021",
         indicator = "Population aged 16-64 years (from best-fit OAs)",
         unit = "Persons") %>%
  arrange(desc(measure)) %>%
  select(area_code, area_name, indicator, period, measure, unit, value)


write_csv(df_16_64, "../data/population_16-64_years.csv")  
