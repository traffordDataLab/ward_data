# Demographics: Percentage of population aged 80 or more years, 2020 #
...
row <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_2010_1.data.csv?geography=1656750701...1656750715,1656750717,1656750716,1656750718...1656750721&date=latest&gender=0&c_age=200,210&measures=20100") 
...
df <- row %>%
  ...
  mutate(Percentage = round(`Aged 85+`/`All Ages`*100, 1)) %>% 
  select(area_code, area_name, Percentage, Count = `Aged 85+`) %>%
