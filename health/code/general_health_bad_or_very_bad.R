# Health: General Health - bad or very bad, 2021 #
...
df <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_2055_1.data.csv?date=latest&geography=629174437...629175131,629304895...629304912,629315184...629315186,629315192...629315198,629315220,629315233,629315244,629315249,629315255,629315263,629315265,629315274,629315275,629315278,629315281,629315290,629315295,629315317,629315327&c2021_health_6=0,4,5&measures=20100") %>%
  ...
  mutate(category= ifelse(category != "Total: All usual residents", "Bad or very bad health", category)) %>%
  group_by(period,area_code, area_name, category) %>%
  summarise(value = sum(value)) %>%
  mutate(Percentage = round(value *100/value[2],1)) %>%
  ungroup() %>%
  filter(category == "Bad or very bad health") %>%
  mutate(indicator = "People reporting their general health as bad or very bad (from best-fit OAs)",
         measure = "Percentage",
         unit = "Persons") %>%
  arrange(desc(measure)) %>%
  select(area_code, area_name, indicator, period, measure, unit, value = Percentage)

write_csv(df, "../data/general_health_bad_or_very_bad.csv")
