# Education: Residents with no qualifications, 2021 #

# Source: TS067 - Highest level of qualification
# URL: https://www.nomisweb.co.uk/datasets/c2021ts067
...
df <- read_csv("https://www.nomisweb.co.uk/api/v01/dataset/NM_2084_1.data.csv?date=latest&geography=629174437...629175131,629304895...629304912,629315184...629315186,629315192...629315198,629315220,629315233,629315244,629315249,629315255,629315263,629315265,629315274,629315275,629315278,629315281,629315290,629315295,629315317,629315327&c2021_hiqual_8=0,1&measures=20100") %>%
  select(period = DATE_NAME,
         OA21CD = GEOGRAPHY_CODE,
         category = C2021_HIQUAL_8_NAME,
         value = OBS_VALUE) %>%
  ...
  mutate(indicator = "Residents with no qualifications (from best-fit OAs)",
         unit = "Persons") %>%
  arrange(desc(measure)) %>%
  select(area_code, area_name, indicator, period, measure, unit, value)


write_csv(df, "../data/residents_no_qualifications.csv")
